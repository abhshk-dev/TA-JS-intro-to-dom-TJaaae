...
let NoOfChildren=document.querySelectorAll('.topics li');
console.log(`number of children =${NoOfChildren.length}`);
...
let print=allTopics.forEach(elm=>(console.log(elm.innerText)));
...
console.log(`Total buttons = ${document.querySelectorAll('input[type="submit"]').length}`);
// 18. Select all the `label` element and log the count.
console.log(`All the label elements = ${document.querySelectorAll('label').length}`);
// 19. Select all the elements with `id` of `test`
let idOfTest=document.querySelectorAll('#test');
// 20. Select the first element with id `test` using `getElementById`
let firstTest=document.querySelector('body');
console.log(firstTest.firstElementChild);
